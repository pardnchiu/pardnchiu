...
### Recent
- **[go-bot (v0.3.1)](https://github.com/pardnchiu/go-bot/releases/tag/v0.3.1)** — Brings Discord's interactive surface to parity with Telegram in Discord-native ways: `SendInput` mounts a Primary Button → Modal flow (Discord's only path to free-text capture from a message context), `SendSelect` and `SendMultiSelect` use String Select Menu dropdowns (`MaxValues=1` and `MaxValues=N` — Discord supports multi-pick natively so the telegram ✅/⬜ toggle + "Done" button machinery isn't needed), `SendVoice` uploads Gemini TTS output as an `audio/ogg` attachment, and `Delete` is added as a thin `ChannelMessageDelete` wrapper for Q&A prompt cleanup. `Input` gains `CallbackPicks []string` so multi-select results land on a dedicated field instead of overloading `Text`.
