...
### Recent
- **[go-utils (v0.9.0)](https://github.com/pardnchiu/go-utils/releases/tag/v0.9.0)** — Add optional sandbox policy injection to the `filesystem` package: `New(Policy{DeniedMap, ExcludeList})` registers JSON-driven deny / exclude rules once via `sync.Once`, exposing `IsDenied` / `IsExcluded` / `RealPath` / `AbsPath` as public utilities. Mutating ops (`WriteFile`, `WriteText`, `WriteJSON`, `AppendText`, `Copy`, `Move`, `Remove`) now consult `IsDenied` internally; read-side and listing APIs are unchanged. Callers that never call `New(...)` see no behavior change — `IsDenied` always returns `false` and `IsExcluded` only reads runtime `.gitignore`.
