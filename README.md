...
### Recent
- **[Agenvoy (v0.20.0)](https://github.com/pardnchiu/Agenvoy/releases/tag/v0.20.0)** — Session lifecycle gets a friendly-name layer, a runtime singleton, and a one-shot routing prefix: each session writes a per-folder `bot.md` (frontmatter `name` + agent persona body, idempotent), CLI gains `agen new`/`agen switch`/`agen config` commands keyed off that name, and `invoke_subagent` accepts `name="<X>"` to dispatch into a named `cli-`/`http-` session — natural-language phrases like "呼叫專業小幫手分析 tsmc" route through this path via a new forced-routing rule. CLI/TUI input gains a `:<name>` prefix (e.g. `:release-bot ship v0.20`) that one-shot-targets a named session without mutating the global pointer; composable with `/skill` and `/external` (resolution order: session → external → skill). Skill prefix routing now passes user-supplied args through as the user message (e.g. `/commit-generate v0.20.0` → user message `v0.20.0`), and `skill_execution.md` adds Mandatory Principle #5 binding the LLM to weave that context into output. Runtime adds a UID/PID singleton (`~/.config/agenvoy/runtime.uid`) that supersedes prior server processes on startup, plus per-session `status.json` (active task array) and rotating `action.log` (1MB, 768KB trim, append-only). `ask_user` now gates on `cli-*` prefix and returns guidance text on other prefixes (subagent / Discord / HTTP have no live stdin). go-utils upgraded to v0.9.4 with tool-event logging refinements.
